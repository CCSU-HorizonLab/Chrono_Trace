...
- [x] direct reply 使用 `【可用历史记忆检索结果】`。
- [x] suggestion 使用 `【联系人级共同记忆 RAG】`。
- [x] no-hit 使用 `【历史记忆检索状态】`。
- [x] prompt 明确要求模型不能编造检索结果之外的历史。
- [x] 最近对话优先级仍高于 RAG。
...
- [x] 扩展固定人工样例集。
- [x] 覆盖明确记忆请求、隐含记忆请求、关系策略请求、普通闲聊、普通话术、no-hit。
- [x] 对比优化前后 RAG 触发、命中、输出质量（当前为旧规则兼容代理；答案质量保留待人工/LLM judge 字段）。
- [x] 记录误触发率、漏触发率、no-hit 诚实率。
