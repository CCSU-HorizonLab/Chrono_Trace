...
- [x] 正常脱敏失败时，使用现场强规则遮罩压缩摘要。
- [x] 不发送长原文。
- [x] 不发送 `sensitive` 记忆。
- [x] 强遮罩失败时阻断远程 RAG 请求，回退无 RAG 或本地/模板。
- [x] 日志记录 `redaction_fallback = true`。
...
- [x] 抽取基础 `shared_memory`。
- [x] 为记忆记录主体、客体、时间、置信度、敏感级别。
- [x] 实现软降权。
- [x] 实现冲突记忆 `superseded` 标记。
- [x] 用户禁用或删除的记忆不得再进入候选。
...
- [x] 设置页展示 RAG 总开关。
- [x] 展示远程 RAG 脱敏开关和风险确认。
- [x] 展示远程 embedding 开关和风险确认。
- [x] 展示索引状态、索引数量、更新时间、占用空间。
- [x] 支持联系人级启用、禁用、清空、重建索引。
...
- [x] 建立匿名/脱敏样例集。
- [x] 覆盖冷场、安抚、邀约、解释、争执降温、共同记忆唤醒、敏感记忆不注入、铺垫后回复。
- [x] 支持无 RAG 与有 RAG 输出对比。
- [x] 记录人工评估维度：策略、记忆、风格、隐私、身份区分。
