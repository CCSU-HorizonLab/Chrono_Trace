...
- [x] 删除 document fallback 的 `_time_scope` / `_within_time_scope` 硬过滤，并确认事实路径不引入等价时间窗。
- [x] 保留事实的 `as_of`、时间衰减和 supersede 链作为时间语义来源。
- [x] 单测：`我刚下班` 不会过滤一周前但与问题直接相关的文档。
- [x] 完成回归测试、更新勾选并提交 `feat：移除RAG刚字时间硬过滤`。
