...
运行记录：评测器、回放、候选导出、NLI 输入导出与映射校验单测 `11 passed`（含安全/隔离指标）；完整 backend 回归 `625 passed, 21 skipped, 24 warnings`；已从 36 条回放生成 108 条脱敏 judge 输入。36 条发布 gold 仍使用 `fact_*` 符号 ID，尚未映射到运行库数字 fact ID；同时没有已判定的答案 judge 文件，发布门禁仍未通过。
