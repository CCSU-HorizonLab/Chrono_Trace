...
**当前进度（2026-09-25 晚，P1 完成 + P2.1 影子闭环 + 并发根治）**：

- P0 除 P0.3 标注执行外全部完成（P0.3 挖掘脚本与标注指南已备，等使用数据到量）；P1 全部完成（关系策略 + 偏好策略独立槽 + 分槽注入）；P2.1 完成记录闭环与 rewritten→偏好候选**影子**写入（uncertain 行不进读侧，待 P0.3 校准转正）；P2.2/P2.3 依赖 P0.3。
- 工程加固四轮（计划外，实测反馈驱动）：P1.5/P1.6 缺口修复、原型路径收紧（<12 字）、索引并发（重建互斥+段级提交）、数据库并发根治（autocommit 消灭挂起事务锁死）。
- 真实库现状（conv 7191）：LLM 事实 122 active + 5 superseded（融合演变链生效）、原型收紧后 41 active、隔离 812；**当前事实池检索自洽性 9/10**（各 kind 抽样、真实 embedding，取代已失效的存活 gold 4/5 口径）。
- 全量后端回归 **745 passed / 21 skipped**。

现行验收口径（P0.3 人工集建立前）：
- **存活检索自洽性**：当前事实池抽样查询的 top-6 命中（9/10，2026-09-25）；
- **人工事实质量抽查**：按用户实测反馈对真实库事实逐条核对（拦截/自包含/证据可溯），记录于 `docs/rag-v4-improvement-plan.md`；
- **全量后端回归**。
